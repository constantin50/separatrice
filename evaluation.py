...
'И ведь есть люди, которые на полном серьёзе думают, что каждому участнику акции платят по 50€.' : ['И ведь есть люди','которые на полном серьёзе думают', 'что каждому участнику акции платят по 50€.'],
...
'где брать справку о том, что я действительно студент вуза?' : ['где брать справку о том', 'что я действительно студент вуза?'],
'я хочу получить справку, которая подтверждает, что я студент ггф!' : ['я хочу получить справку','которая подтверждает','что я студент ггф!'],
...
'для любого эпсилона большего нуля существует такая дельта больше нуля, что \"хвост\" функции не будет выходить за границы этого эпсилона.' : ['для любого эпсилона большего нуля существует такая дельта больше нуля', 'что \"хвост\" функции не будет выходить за границы этого эпсилона.'],
...
'да, верно, но есть проблема в том, что эта штука работает нестабильно, а результаты разные из-за разных моделей.' : ['да, верно', 'есть проблема в том', 'что эта штука работает нестабильно', 'результаты разные из-за разных моделей.'],
...
'и ещё, пиши раз в день, пожалуйста, о прогрессе, например, \"за сегодня было добавлено столько-то фраз такого типа и столько-то синсетов было наполнено\".' : ['и ещё, пиши раз в день, пожалуйста, о прогрессе, например', '\"за сегодня было добавлено столько-то фраз такого типа' ,'и столько-то синсетов было наполнено\".'],
 'Ребят, где находится магаз?':['Ребят', 'где находится магаз?'],'Иван Васильевич, а что сказал Петр Иванвович?':['Иван Васильевич', 'что сказал Петр Иванвович?'],'Чел, ты потерялся...':['Чел','ты потерялся...'],'Иван, Петр сказал, что ты не очень прилежный студент!':['Иван', 'Петр сказал', 'что ты не очень прилежный студент!']}
